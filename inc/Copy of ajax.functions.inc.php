...
function print_menu($lsn)
{
	...
	$menu.= sprintf($button, 'Search', 'xajax_search_bugs(lsn)');
...
function my_profiles($lsn)
{
	...
	$page.= '<h1>Filters</h1>';
	$page.= '<table border=1>';
	dbq("SELECT profileName, profileData FROM profiles WHERE userID=$userID and profileType='filter'");
	while ( $row = dbrow() )
	{
		$page.= '<tr>';
		$page.= '<th>'.$row['profileName'].'</th>';
		$page.= '<td>'.$row['profileData'].'</td>';
		$page.= '<td><input type="button" value="Remove"></td>';
		$page.= '</tr>';
	}
	$page.='</table>';
...
function print_filter_nice($filterID, $profileData, $editable = false) // this is not an ajax exported function
{
	dbconnect();
	$txt = '';
...
// );
	...
	$profile_info.= '</select>';
	if ( $userID != $profileOwnerID )
		$profile_info = '<input type="button" class="default" value="Save profile as" onClick="xajax_save_profile_as(lsn, '.$filterID.', document.getElementById(\'save_filter_as\').value)"><input type="text" class="profile_name" value="'.$profileName.'" id="save_filter_as"><br>'.$profile_info;
	...
	foreach ( $profileData as $rowID=>$row )
	{
		...
		foreach ( $expr as $rowFilterID=>$filters )
		{
			...
			$first_expr = false;
			$t.= select($all_filters, $a, 'operand1', sprintf($onChange, 0)).select($operators2, $op, 'operator2', sprintf($onChange, 1)).select($all_filters_val[$a], $b, 'operand2', sprintf($onChange, 2));
		}
		if ( !empty($t) )
		{
			$txt.= '<tr><td>';
...
// $profileData = array('ID','title','assignedToName','closedByName','severityName');
	...
	$possible_cols = array( // there is a copy of this definition in function print_bug_table
		//		field name				what we print			what we query for
		array(	'ID',				'#',				'ID'),
		array(	'platformName',		'Platform',			'(SELECT platformName FROM platforms WHERE platforms.ID=platformID) AS platformName'),
		array(	'frequencyName',	'Frequency',		'(SELECT frequencyName FROM frequency WHERE frequency.ID=frequencyID) AS frequencyName'),
		array(	'frequencyPercent',	'Frequency%',		'frequencyPercent'),
		array(	'title',			'Title',			'title'),
		array(	'assignedToName',	'Assigned to',		'(SELECT login FROM users WHERE users.ID=assignedTo) AS assignedToName'),
		array(	'assToGroupName',	'Group assigned',	'(SELECT groupName FROM groups WHERE groups.ID=assToGroup) AS assToGroupName'),
		array(	'closedByName',		'Closed by',		'(SELECT login FROM users WHERE users.ID=closedBy) AS closedByName'),
		array(	'severityName',		'Severity',			'(SELECT severityName FROM severity WHERE severity.ID=severityID) AS severityName'),
		array(	'statusName',		'Status',			'(SELECT statusName FROM status WHERE status.ID=statusID) AS statusName'),
		array(	'typeName',			'Type',				'(SELECT typeName FROM type WHERE type.ID=typeID) AS typeName'),
		array(	'openDate',			'Open date',		'openDate'),
		array(	'closeDate',		'Closed date',		'closeDate'),
		array(	'submitedDate',		'Submited date',	'submitedDate'),
		array(	'versionDate',		'Build',			'versionDate'),
	);
...
function print_bug_table($lsn, $limitStart = 0, $customWhere = '')
{
	$objResponse = new xajaxResponse();
	$strip = stripLSN($lsn);
	if ( empty($strip) )
...
// $displayData = array('ID', 'severityName', 'ID', 'title', 'assignedToName', 'closedByName', 'severityName');
	...
	$q = dbq("SELECT $display_txt (SELECT severityColor FROM severity WHERE severity.ID=severityID) AS severityColor, ID, assignedTo, flags FROM bugs$defaultApp WHERE $filter_txt");
	...
	while ( $row = dbrow() )
	{
		...
		foreach ( $displayData as $val )
		{
			...
			if ( $val == 'title' )
				$row[$val] = '<a class="bug" target="_blank" href="'.$thisPageURI.'?bug='.$row['ID'].'&app='.$defaultApp.'">'.$row[$val].'</a>';
			$page.= '<td>'.$row[$val].'</td>';
		}
		$page.= '</tr>';
	} 
	$page.= '</tbody>';
	$page.= '<tfoot><tr><th colspan="'.(count($header) + 1).'">Displaing '.($limitStart + 1).' - '.($limitStart + $bugsPerPage - max(0, $displayBugs)).' of '.$totalBugs.' bugs matching your filters</th></tr></tfoot>';
	$page.= '</table>';
...
function print_bug($lsn, $bugID, $appID = 0)
{
	$objResponse = new xajaxResponse();
	$strip = stripLSN($lsn);
	$bugID = (int)$bugID;
	...
	dbq("UPDATE bugs$appID SET flags=(flags | ".BUG_VIEWED.") WHERE ID=$bugID AND assignedTo=$userID");
	$q = dbq("SELECT $select statusID, assToGroup, assignedTo, description, info, notes, history, versionDate, (SELECT appName FROM apps WHERE ID=$appID) AS appName, (SELECT login FROM users WHERE users.ID=submitedBy) AS submitedByName FROM bugs$appID WHERE ID=$bugID");
	if ( ($x = mysql_num_rows($q)) != 1 )
	{
		$objResponse->alert("Bug '$bugID' not found");
		return $objResponse;
	}
	$bug = dbrow();
	// foreach ($bug as $h=>$b)
	...
	$page.= '<tr><td colspan="4"><h1>Bug '.$bug['ID'].'</h1></td></tr>';
	$page.= '<tr><td><table>';
	$page.= '<tr><td><em>Application</em></td><td>'.$bug['appName'].'</td></tr>';
	$page.= '<tr><td><em>Severity</em></td><td>'.$bug['severityName'].'</td></tr>';
	...
	if ( $bug['statusID'] != BUG_STATUS_CLOSED )
	{
		...
		if ( empty($bug['assignedTo']) && empty($bug['assToGroup']) )
			$page.= 'Please assign this bug';
		else
			$page.= '<input type="button" value="Close bug" class="default" onClick="if(confirm(\'Are you sure you want to close this bug?\')'.$confirm2.'){xajax_close_bugs(lsn, '.$appID.', '.$bugID.');xajax_print_bug(lsn, '.$bugID.', '.$appID.')};">';
	}
	$page.= '</td></tr><tr><td>';
	...
	// $page.= '<tr><td><em>Application</em> '.$bug['appName'].'</td><td><em>Platform</em> '.$bug['platformName'].'</td><td><em>Build</em> '.date(DATE_BUGVER, $bug['versionDate']).'</td></tr>';
	// $page.= '<tr><td><em>Severity</em> '.$bug['severityName'].'</td><td><em>Status</em> '.$bug['statusName'].'</td><td><em>Frequency</em> '.$bug['frequencyName'].' '.$bug['frequencyPercent'].'</td></tr>';
	// $page.= '<tr><td><em>Opened</em> on '.date(DATE_NOTIME, $bug['openDate']).'</td><td>'.$bug['closeDate'].'</td><td>'.$bug['closedByName'].'</td></tr>';
	// $page.= '<tr><td>'.$bug['assign'].'</td><td><em>Type</em> '.$bug['typeName'].'</td><td>'.$bug['submited'].'</td></tr>';
...
function view_bugs($lsn)
{
	...
	$page.= '<div class="bug_table" id="bug_table_div">Press "'.$refresh_btn.'" to load the bugs. If the button does not work the probable cause is that your browser can not handle the amount of bugs in the list. Set less bugs per page or put more filters.<br>If you are sure this is not the case please inform the webmaster about the problem.</div>';
...
function edit_all($lsn)
{
	...
	for ( $i = 0; $i < $nSize; $i += 1 )
	{
		...
		$page.= '<a class="bug" target="_blank" href="'.$thisPageURI.'?bug='.$bugs[$i][1].'&app='.$bugs[$i][0].'">'.$bugs[$i][1].'</a>, ';
		$theBugs.= ', '.$bugs[$i][1];
...
function add_bugs($lsn)
{
	...
	if ( func_num_args() == 1 )
	{
		...
		$page.= '<tr><th>Severity</th><td>';
		$page.= '<select id="add_bug_severity"><option value="0">Choose severity</option>'.$theSeverity.'</select>';
		$page.= '</td></tr>';
		...
		$page.= '<input id="add_bug_version" type="text"> (MM/DD/YY)';
		$page.= '</td></tr>';
	...
	else
	{
		...
		dbq("INSERT INTO bugs$args[1] (openDate, assToGroup, assignedTo, typeID, severityID, platformID, versionDate, frequencyID, frequencyPercent, title, description, statusID) 
			VALUES (".time().", $args[2], $args[3], $args[4], $args[5], $args[6], $args[7], $args[8], '$args[9]', '$args[10]', '$args[11]', ".BUG_STATUS_OPEN.")");
...
function search_bugs($lsn)
{
	...
	$nSize = count($searchIn);
	if ( func_num_args() > $nSize )
	{
		$page.= '<h1>Results</h1>';
		$page.= '<input type="button" class="default" value="Edit selected" onClick="get_checkboxes(\'bug_table\', \'edit_all\')">';
	...
	$page.= '<tr><td>';
	$page.= 'for <input type="text" id="search_text" value="'.$text.'"> in';
	$page.= '</td><td>';
	...
	$page.= '</td><td>';
	$page.= '<input type="button" class="default" value="Search" onClick="xajax_search_bugs(lsn,';
	foreach ( $searchIn as $search )
	{
		list ($name, $echo, $chk) = $search;
	...
	$page.= ')"> (use % in order to elapse characters)</td></tr>';
	$page.= '</table>';
	...
	{
		$objResponse->loadCommands(print_filters($lsn));
		$objResponse->loadCommands(print_bug_table($lsn, 0, $customWhere));
	}
	return $objResponse;
}
